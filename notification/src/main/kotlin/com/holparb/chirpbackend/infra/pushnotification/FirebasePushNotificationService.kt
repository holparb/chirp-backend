package com.holparb.chirpbackend.infra.pushnotification
...
import com.holparb.chirpbackend.domain.model.PushNotificationSendResult
...
class FirebasePushNotificationService(
    @param:Value("\${firebase.credentials-path}") private val firebaseCredentialsPath: String,
    ...
    fun initialize() {
        try {
            val serviceAccount = resourceLoader.getResource(firebaseCredentialsPath)
    ...
    fun tokenValid(token: String): Boolean {
        ...
        return try {
            ...
            logger.warn("Failed to validate firebase message with token", e)
            false
        }
    }
                    ...
                    when(recipient.platform) {
                        DeviceToken.Platform.ANDROID -> setAndroidConfig(
                            AndroidConfig.builder()
                                ...
                                .setRestrictedPackageName("com.holparb.chirp")
                                .build()
                        )
